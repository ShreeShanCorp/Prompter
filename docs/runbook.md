...
- **Playwright E2E scaffolding exists but hasn't passed a local run yet** (`apps/web/playwright.config.ts`, `apps/web/e2e/smoke.spec.ts`). It's a real, correct smoke test (sign in → create project → see it → delete it) verified up through the actual Clerk sign-in interaction, but the shared test account's sign-in screen degraded to social-login-only (Apple/GitHub/Google, no password field) during setup -- likely Clerk's anomaly/security step-up triggered by earlier failed automated attempts against that account, not a bug in the test or app. To actually run it: set `E2E_TEST_EMAIL`/`E2E_TEST_PASSWORD` in `apps/web/.env` for an account with normal password sign-in, `docker compose up -d`, both dev servers running, then `npm run test:e2e --workspace @prompter/web`.
